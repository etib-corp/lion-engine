    ...
    switch (type) {
        case FileType::WAV:
            if (!drwav_init_file(&this->_wav, path.c_str(), NULL))
                throw SoundError("Can not open wav file : [" + path + "]", "Check if the file is a valid wav file or if the file exists");
            ...
            if (this->_wav.channels == 1)
                ...
                throw SoundError("Invalid channel count : [" + path + "]", "Check if the file is a valid wav file");
        ...
        case FileType::MP3:
            if (!drmp3_init_file(&this->_mp3, path.c_str(), NULL))
                throw SoundError("Can not open mp3 file : [" + path + "]", "Check if the file is a valid mp3 file or if the file exists");
            ...
            if (this->_mp3.channels == 1)
                ...
                throw SoundError("Invalid channel count : [" + path + "]", "Check if the file is a valid mp3 file");
            alGenBuffers(1, &this->_buffer);
            alBufferData(this->_buffer, format, samples.data(), samples.size() * sizeof(int16_t), this->_mp3.sampleRate);
            break;
        default:
            throw SoundError("Unknown file type : [" + path + "]", "Check if the file is a valid sound file");
            break;
    }
