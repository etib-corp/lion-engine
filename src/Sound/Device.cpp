...
LE::Sound::Device::Device()
{
    switch (InitOpenALDevice(this)) {
        case LE::Sound::Device::ErrorType::CAN_NOT_OPEN_DEVICE:
            throw LE::Sound::Device::SoundDeviceError("Can not open device (default)", "Check if the device is available");
            break;
        case LE::Sound::Device::ErrorType::CAN_NOT_CREATE_CONTEXT:
            throw LE::Sound::Device::SoundDeviceError("Can not create context", "Check if the device is available or if the context is already created");
            break;
        case LE::Sound::Device::ErrorType::CAN_NOT_MAKE_CONTEXT_CURRENT:
            throw LE::Sound::Device::SoundDeviceError("Can not make context current", "Check if the context is already created");
            break;
        default:
            break;
    }
